...
`ready-for-agent` maps to `Sandcastle` because the Sandcastle orchestrator (`.sandcastle/`) picks up issues by that label — triaging an issue as AFK-ready is the same act as queueing it for Sandcastle.
